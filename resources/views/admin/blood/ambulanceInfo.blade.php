    ...
    <div class="m-portlet">
        <div class="m-portlet__head">
            <div class="m-portlet__head-wrapper">
                ...
                <a href="{{ route('admin.ambulance.create') }}" class="btn btn-success m-btn m-btn--icon m-btn--wide m-btn--md m--margin-right-10">
                        <span>
                    ...
                    <!-- import button-->
                    <a href="{{ route('admin.ambulance.import') }}" class="btn btn-primary m-btn m-btn--icon m-btn--wide m-btn--md m--margin-right-10">
                        <span>
                            <i class="fa fa-upload"></i>
                            <span>Import</span>
                        </span>
                    </a>
                    <a href="{{ route('admin.ambulance.export') }}" class="btn btn-primary m-btn m-btn--icon m-btn--wide m-btn--md m--margin-right-10">
                        <span>
                            <i class="fa fa-upload"></i>
                            <span>Export</span>
                        </span>
        ...
        <div class="m-portlet__body">
            <div class="m-section">
                ...
                <!-- Search Bar -->
                <form action="{{ route('ambulance-show') }}" method="GET">
                    <div class="form-row align-items-center mb-3">                                               
                        ...
                        <div class="col-auto">
                            ...
                            <input type="text" class="form-control" id="name" name="name" placeholder="Search by Sewa Name">
                        </div>
                        <div class="col-auto">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover" id="ambulanceTable">
                        <thead>
                            ...
                            <tbody>
                                @foreach ($ambulanceInfo as $requests)
                                <tr class="dd">
                                    <td>{{ $requests->ambulanceInfoId }}</td>
                                    <td>{{ $requests->name}}</td>
                                    <td>{{ $requests->contactNo}}</td>                                   
                                    <td>{{ $requests->localLevel }},- {{ $requests->wardNo }}, {{ $requests->district }}, Pro. {{ $requests->province }}</td>                                
                                    <td style="text-align: center; vertical-align: middle;">
                                        <form method="POST" action="{{ route('ambulance-delete', ['id' => $requests->ambulanceInfoId]) }}">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="12">
                                        ...
                                        <div class="pagination justify-content-center">
                                            {{ $ambulanceInfo->onEachSide(2)->links() }}
                                        </div>
                                    </td>
                                </tr>
                            </tfoot>
